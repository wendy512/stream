 ...
 * @date 2023-05-25 14:19:36
 * @since 1.0.0
 */
public class RabbitMqSender extends AbstractLifecycleAware {

    private static volatile RabbitMqSender instance;

    private final AbstractProperties properties;

    private final RabbitMqStateConfigure stateConfigure;
    ...
    private RabbitMqSender(AbstractProperties properties) {
    ...
    public static RabbitMqSender getInstance(AbstractProperties properties) {
        if (null == instance) {
            synchronized (RabbitMqSender.class) {
                if (null == instance) {
                    instance = new RabbitMqSender(properties);
                    instance.start();
                }
            }
        }
